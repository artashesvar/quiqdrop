...
# Limit dedup to function words — "very very good" is intentional emphasis; only stutter-style repeats in function words are accidents
...
def _remove_fillers(text: str) -> str:
    ...
    # "so, um, the plan" → remove "um" → "so, , the plan" → fix to "so, the plan"
...
def _normalize_whitespace(text: str) -> str:
    ...
    text = re.sub(r",\s*([.!?])", r"\1", text)         # "good,." → "good."
    text = re.sub(r"\s([,.!?])", r"\1", text)           # "good ." → "good."
    text = re.sub(r"([.!?])([A-Z])", r"\1 \2", text)   # "end.Start" → "end. Start"
    return text
